...
class Transfert(models.Model):
    ...
    token_validation = fields.Char(string="Clé de vérification")
    ...
    Sexe_expediteur = fields.Selection(related='id_expediteur.sexe',store=True,  string="Sexe expediteur")
    ...
    def _message_envoie(self):
        ...
        if self.state1=="envoye":
            ...
            nom2 = self.name_beneficiaire
            self.message= "M "+nom2+" a faire un retrait de votre part à la Bange Bank à "+str(fields.Datetime.now())+"."+"\n Bange Bank Votre banque de confiance."
        else:
            self.message= ''

    @api.depends('tel_beneficiaire', 'state1')
    def _numtelephone_envoie(self):
        ...
        tel_exp= self.tel_expediteur
        cr =self._cr
        id_user = self.env.uid
        cr.execute( 'select mp_pays.name from mp_pays '
                    'inner join mp_ville mp_v on mp_v.id_pays = mp_pays.id '
        ...
        #id_user = self.env.uid
        #cr.execute('select mp_transfert.state1, mp_transfert.id from mp_transfert '
        #            'where mp_transfert.id =  ' +str(id_user)+'')
        #result = cr.fetchall()
        #status = result[0][0]
        #print(status)
        ...
        if self.state1 == "envoye":
            if pays == "Guinnée Equatoriale":
                self.recipient = "240" + str(tel)

        elif self.state1 == "retirer":
            if pays == "Guinée équatoriale":
                if self.token_validation!= "":
                    self.recipient = "240"+str(tel_exp)
        elif self.state1 == "retirer":
            if pays == "Cameroun":
                    if self.token_validation!= "":
                        self.recipient = "237" + str(tel_exp)
        else:
            print("Vous avez pas choisi le pay d'envoi")
            self.recipient = "vous avez pas choisi de pays "
    ...
    def onchange_montant(self):
        ...
        if self.montant != 0:
                ...
                cr.execute( 'select id,commission,mode_calcul from mp_tarif where montant_min <= '+"'"+str(self.montant)+"'"+' AND montant_max >= '+"'"+str(self.montant)+"'"+' AND id_pays_depart =  ' +str(id_depart)+' AND id_pays_arrive =  ' +str(id_arrive)+'')
                result = cr.fetchall()
                if result !=[]:
                    id_tarif = result[0][0]
    ...
    def _compute_commission(self):
        ...
        for record in self:
                if record.id_ville_arrive.id != False :
                    ...
                    result = cr.fetchall()
                    id_arrive = result[0][0]
                    cr.execute( 'select commission from mp_tarif where montant_min <= '+"'"+str(record.montant)+"'"+' AND montant_max >= '+"'"+str(record.montant)+"'"+' AND id_pays_depart =  ' +str(id_depart)+' AND id_pays_arrive =  ' +str(id_arrive)+'')
                    result = cr.fetchall()
                    if result !=[]:
    ...
    def _compute_id_tarif(self):
        ...
        for record in self:
            if record.id_ville_arrive.id != False:
                ...
                cr.execute( 'select id from mp_tarif where montant_min <= '+"'"+str(record.montant)+"'"+' AND montant_max >= '+"'"+str(record.montant)+"'"+' AND id_pays_depart =  ' +str(id_depart)+' AND id_pays_arrive =  ' +str(id_arrive)+'')
                result = cr.fetchall()
                if result !=[]:
                    id_tarif = result[0][0]
                    record.id_tarif = id_tarif
                else:
                    cr.execute( 'SELECT id FROM mp_tarif ORDER BY id DESC LIMIT 1')
                    result = cr.fetchall()
                    id_tarif = result[0][0]
                    record.id_tarif = id_tarif
            else:
    ...
    def _compute_montant_total(self):
        ...
        if self.id_ville_arrive.id != False :
            ...
            for record in self:
                    ...
                    cr.execute( 'select mode_calcul from mp_tarif where montant_min <= '+"'"+str(record.montant)+"'"+' AND montant_max >= '+"'"+str(record.montant)+"'"+' AND id_pays_depart =  ' +str(id_depart)+' AND id_pays_arrive =  ' +str(id_arrive)+'')
                    result = cr.fetchall()
                    if result !=[]:
    ...
    def envoyer(self, context=None):
        ...
        mp_operation = self.env['mp.operation'].create({
            'name': 'depot',
    ...
    def retrait(self, context=None):
        ...
        if result[0][0] == None or result[0][1] == None:
            raise ValidationError("Champs non remplis, veillez entrer la le type de pièce d'identification et le numéro de la pièce d'identification du bénéficiaire")
        else:
            if self.token == self.token_validation :
                id_user = self.env.uid
                cr =self._cr
                ...
                self.write({'token_validation':' '})
                raise ValidationError("Clé de validation incorrecte, Veillez vérifier la clé de validation")
